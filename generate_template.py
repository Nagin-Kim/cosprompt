...
def get_text(template, input_text_tuple, label, tokenizer, mapping):
    ...
    special_token_mapping = {'cls': tokenizer.cls_token_id, 'mask': tokenizer.mask_token_id, 'sep': tokenizer.sep_token_id, 'sep+': tokenizer.sep_token_id} # 特殊标签
    for i in range(10): # 对于0-9
        special_token_mapping["<extra_id_%d>" % (i)] = tokenizer._convert_token_to_id("<extra_id_%d>" % (i))  # 对新加入的extra_id也给一个id
    template_list = template.split('*') #将templist分割一下         template = "*cls**sentu_0**<extra_id_0>**label**<extra_id_1>**sep+*"
    input_ids = []
...
def generate(dataset, template, model, tokenizer, target_number, mapping, beam, label=None, length_limit=None, truncate=None):
    ...
    # Process the inputs
    for item in dataset:  #对于每一条数据
        if label is None or item['label'] == label: #如果label未设置为none 或者当前item不是为指定的label
            input_text = get_text(template, item['text'], item['label'], tokenizer, mapping)  # 获取input text 要去组装
